...
class TestModuliAssembly(TestCase):
    ...
    def test_create_candidate_path(cls):
        ...
        key_length = 2048
        tpath = '/'.join((str(cls.ma_real.config['moduli_dir']), f'{key_length}.candidate_'))
        cls.assertTrue(tpath in str(cls.ma_real.create_candidate_path(key_length)))
    ...
    #
    #     # Save Candidate to test/resources to load for Moduli Creation
    ...
    #     moduli_file = cls.ma.create_moduli_file()
    #     cls.assertTrue(moduli_file.exists())
    #     cls.assertTrue(moduli_file.stat().st_size > 1)
    #
    # def test_write_named_moduli_file(cls):
    #     moduli_file = cls.ma.create_moduli_file(cls.ma.config['config_dir'].joinpath('TEST_MODULI_FILE'))
    #     cls.assertTrue(moduli_file.exists())
    #     cls.assertTrue(moduli_file.stat().st_size > 1)
    #
